...
def preprocess_doc(doc, stop_words):
    ...
    doc = " ".join([lemmatizer.lemmatize(w) for w in doc.split() if w not in stop_words])
...
def write_csv(labels, titles, contents, mode="train"):
    f = open("dbpedia_csv/" + mode + "_preprocessed.csv", "w")
...
def write_pkl(labels, titles, contents, mode="train"):
    fname = "dbpedia_csv/" + mode + "_preprocessed_vectorized.pickle"
